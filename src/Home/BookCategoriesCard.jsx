...
const BookCategoriesCard = ({ oneCategories }) => {
    const { category, color, icon, description, quantity, label, _id, cover_image } = oneCategories;
    console.log(cover_image)

    return (
        <Link to={`/categories/${_id}`}>
            <div className='hover:scale-105 transform transition duration-300'>
                {/* <div className={` h-37 rounded-t-2xl relative`}>
                ...
                <img className='w-full h-50 rounded-t-xl object-cover' src={cover_image} alt="" />
